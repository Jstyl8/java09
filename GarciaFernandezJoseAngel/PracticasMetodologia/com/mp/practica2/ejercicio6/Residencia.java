...
public class Residencia {
	...
	public Residencia(String nombre, Cuidador[] cuidadores,
		...
		for (int i = 0; i < cuidadores.length; i++) {
			if (cuidadores[i] != null) {
				this.cuidadores[i] = cuidadores[i];
				nCuidadores++;
			}
		}
	...
	public void insertarCuidador(Cuidador cuidador) {
		...
		boolean Esta = false;
		for (int i = 0; (i < nCuidadores) && (Esta == false); i++) {
	...
	public void insertarHabitacion(Habitacion habitacion) {
		...
		for (int i = 0; (i < nHabitaciones) && (noAñadir == false); i++) {
	...
	public void insertarResidente(Residente residente) {
		...
		for (int i = 0; (i < nResidentes) && (noAñadir == false); i++) {
			if (residentes[i].equals(residente)) {
				noAñadir = true;
			}
		}
		if (noAñadir == false) {
			if (residentes.length == nResidentes) {
	...
	public void insertarReserva(Reserva reserva) {
		...
		for (int i = 0; (i < nReservas) && (noAñadir == false); i++) {
	...
	public void cambiarHabitacion(Residente residente, Habitacion habitacion,
			Date fechaCambio, Date fechaSalida) {

		// busca la reserva del cliente y actualiza la fecha de salida
		for (int i = 0; i < nReservas; i++) {
			if (reservas[i].getResidente().equals(residente)) {
				reservas[i].setFechaSalida(fechaCambio);
			}
		}

		Reserva reserva = new Reserva(residente, habitacion, fechaCambio,
				fechaSalida);
		insertarReserva(reserva);
	}
	...
	public void ordenarReserva() {
		for (int p = 1; p < nReservas; p++) {
			Reserva temp = reservas[p];
			int j = p;
			for (; j > 0
					&& (temp.getResidente().compareTo(
							reservas[p - 1].getResidente()) < 0); j--)
	...
	public String listadoResidentesHabitaciones(Date fecha) {
		...
		for (int j = 0; j < nReservas; j++) {
			...
			if (reservas[j].getFechaEntrada().compareTo(fecha) <= 0
				...
				listado += " " + reservas[j].getResidente().getNombre();
			}
